...
| # | 你的决定 | 对方案的影响 |
| --- | --- | --- |
| Q1 | **私有仓库** | Actions 分钟数是**账号级共享额度**，重型层不能无脑 nightly——见下方预算表 |
...
**Actions 分钟预算（私有仓库，账号共享）**——三档可选：

| 方案 | nightly（L2 + L3-quick） | weekly（完整 L3 + 60min 浸泡） | 合计/月 | 说明 |
| --- | --- | --- | --- | --- |
| **A 每天** | 20 min × 30 = 600 | 90 min × 4.3 ≈ 390 | **≈ 990** | 覆盖最密，占额度约一半（Free 2000/月） |
| **B 一三五（推荐）** | 20 min × 13 = 260 | 390 | **≈ 650** | 仍每天有夜间回归的"隔天版"，给其他私有仓库留余量 |
| **C 只 weekly** | — | 390 | **≈ 390** | 最省；夜间的部署面回归消失 |

（PR/push 的单元+契约层约 2 分钟/次，按 20 次/月 ≈ 40 分钟，三档都已包含在余量里。）

**待你一句话**：选 A / B / C？（我的建议是 **B**：一三五跑部署面+故障，周日跑全量与 60 分钟浸泡；任何时候
