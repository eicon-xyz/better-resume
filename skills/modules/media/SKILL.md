...
2. 句池归并是纯函数（apd/rpl/时间重叠+演化/尾缀合并），**单字重叠不合并**（MIN_OVERLAP=2，M4 P2）。
   **边界（2026-09 真机实测）**：`evolves()` 只认演化型包；百炼实时的句中改写（`第二句。街`→`第二句街口响`）
...
- worklet 分支只在真浏览器验证过回退路径；**讯飞真机仍未验证**（缺凭据，M4 P0）；百炼实时已真机验证
  （2026-09，证据在 `docs/tickets/p1-post-v/`）。
- 实时通道收尾：`close_timeout` 必须显式（默认 10s，供应商不回关闭帧 → 松手后干等 10 秒；
  详见 `docs/tickets/p1-post-v/PROBLEMS.md`）。
